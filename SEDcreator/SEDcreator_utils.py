...
# Renumber visible SED cameras
def renumberSEDCameras(context):
    cameras = getSEDCameras(context)
    number_cam = 0
    for cam in cameras:
        # Check if the Camera_{number_cam} already exists for renumbering correctly
        new_name = f"Camera_{number_cam}"
        existing_camera = bpy.data.objects.get(new_name)
        if existing_camera:
            bpy.data.objects[new_name].name = "Camera_not_visible"
        cam.name = new_name
        number_cam += 1
